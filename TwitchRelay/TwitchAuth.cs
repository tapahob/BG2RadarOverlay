...
/// <summary>
/// Persists the broadcaster's OAuth tokens for the Channel Points / EventSub path - obtained once
/// via GET /oauth/authorize -> /oauth/callback, refreshed automatically before they expire.
///
...
/// configuration.set(), Bits. Calling Helix / creating an EventSub subscription on the
/// broadcaster's behalf needs a normal OAuth "Application" registered separately, with its own
/// client id/secret (TWITCH_CLIENT_ID / TWITCH_CLIENT_SECRET) and an OAuth Redirect URL pointing
/// at this relay's /oauth/callback.
/// </summary>
public sealed class TokenStore
{
    private readonly string path;
    private readonly SemaphoreSlim gate = new(1, 1);
    private StoredTokens? cached;
    ...
    /// <summary>
    /// A valid access token, refreshing first if it's expired or close to it. Null if nobody has
    /// completed /oauth/authorize yet (or the refresh token has since been revoked) - callers
    /// treat that as "Channel Points isn't set up", not as an error to surface to a viewer.
    /// </summary>
    public async Task<string?> GetValidAccessTokenAsync(HttpClient http, string clientId, string clientSecret)
    {
        await gate.WaitAsync();
        try
        {
            ...
            {
                // The refresh token itself can be revoked (she removed the app's access, or reset
                // her password) - there is no recovering from here without /oauth/authorize again.
                return null;
            }

            using var doc = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
            var root = doc.RootElement;
            var newTokens = new StoredTokens
            {
                AccessToken = root.GetProperty("access_token").GetString() ?? "",
                RefreshToken = root.TryGetProperty("refresh_token", out var r) ? (r.GetString() ?? cached.RefreshToken) : cached.RefreshToken,
    ...
    private async Task persistAsync(StoredTokens tokens)
    {
    ...
    private async Task<StoredTokens?> loadAsync()
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<StoredTokens>(await File.ReadAllTextAsync(path));
        }
        catch (JsonException)
        {
            return null;
